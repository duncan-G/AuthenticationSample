...
function generateTsContent(constants) {
  ...
  lines.push('// To regenerate, run: node scripts/development/generate_error_codes.js');
  lines.push('');
...
async function main() {
  ...
  const csRoot = path.join(repoRoot, 'libraries', 'Exceptions');
  ...
  if (files.length === 0) {
    throw new Error(`No *ErrorCodes.cs files found in ${csRoot}`);
  }
