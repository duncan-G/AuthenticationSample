    ...
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "$LOG_FILE",
            "log_group_name": "/aws/ec2/${PROJECT_NAME}-docker-manager${ENV:+-${ENV}}",
            "log_stream_name": "{instance_id}",
            "timestamp_format": "%Y-%m-%d %H:%M:%S"
          },
          {
            "file_path": "/var/log/leader-manager/leader-manager.log",
            "log_group_name": "/aws/ec2/${PROJECT_NAME}-leader-manager${ENV:+-${ENV}}",
            "log_stream_name": "{instance_id}",
            "timestamp_format": "%Y-%m-%d %H:%M:%S"
          },
          {
            "file_path": "/var/log/certificate-manager/certificate-manager.log",
            "log_group_name": "/aws/ec2/${PROJECT_NAME}-certificate-manager${ENV:+-${ENV}}",
            "log_stream_name": "{instance_id}",
            "timestamp_format": "%Y-%m-%d %H:%M:%S"
          }
        ]
