...
public static class AwsOptionsExtensions
{
    ...
    public static AWSOptions AddAwsOptions(this IHostApplicationBuilder builder, string ssoClientName)
    {
        ...
        var awsOptions = new AWSOptions { Region = RegionEndpoint.GetBySystemName(awsRegion), Profile = awsProfile };
        ...
        if (builder.Environment.IsDevelopment())
        {
            ...
            catch (Exception ex)
            {
                throw new InvalidOperationException(
                    $"Failed to load AWS SSO credentials for profile '{awsOptions.Profile}'",
                    ex);
            }
        }
